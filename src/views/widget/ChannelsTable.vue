<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="category_di" label="一级分类id"></el-table-column>
      <el-table-column prop="group" label="组名"></el-table-column>
      <el-table-column prop="group_id" label="组id"></el-table-column>
      <el-table-column prop="sequence" label="展示顺序"></el-table-column>
      <el-table-column prop="url" label="频道地址"></el-table-column>
      <el-table-column label="操作" width="160">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="fnDelChannel(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    fnPopShow(id) {
      ...
      this.edit_id = id;
      this.axios
        .get(cons.apis + "/goods/channels/" + this.edit_id + "/", {
          headers: {
    ...
    submitForm() {
      this.axios
        .put(
          cons.apis + "/goods/channels/" + this.edit_id + "/",
          {
    ...
    fnDelChannel(id) {
      ...
      this.$confirm("此操作将删除该频道,是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .delete(cons.apis + "/goods/channels/" + this.edit_id + "/", {
              headers: {
                Authorization: "JWT" + token,
              },
              responseType: "json",
