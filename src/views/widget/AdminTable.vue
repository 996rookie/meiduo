      ...
      <el-table-column label="操作" min-width="25">
        <template slot-scope="scope">
          <el-button size="mini" @click="fnPopShow(scope.row.id)"
            >修改</el-button
          >
          <el-button size="mini" type="danger" @click="fnDelAdmin(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="权限">
          <el-transfer
            v-model="adminForm.listval2"
            :data="adminForm.list2"
            size="mini"
          ></el-transfer>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  data() {
    var validateUserName = (rule, value, callbcak) => {
      if (value === "") {
        callbcak(new Error("用户名不能为空!"));
      } else {
        var reUser = /^\w{5,20}$/;
    ...
    var validatePassWord = (rule, value, callback) => {
      if (value === "") {
        ...
        if (rePass.test(value)) {
          ...
          callback(
            new Error("密码是8到20位数字、字母或下划线以及特殊!@#$%^&*符号")
          );
        }
      }
    };
  ...
  methods: {
    fnPopShow(id) {
      this.pop_show = true;
      this.axios
        .get(cons.apis + "/permission/admins" + id + "/", {
          headers: {
    ...
    fnDelAdmin(id) {
      ...
      this.$confirm("此操作将删除该管理员,是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .delete(cons.apis + "/permission/admins/" + this.edit_id + "/", {
              headers: {
                Authorization: "JWT" + token,
              },
              responseType: "json",
    ...
    fnGetGroup() {
      this.axios
        .get(cons.apis + "/permission/groups/simple/", {
          headers: {
            Authorization: "JWT" + token,
          },
          responseType: "json",
    ...
    submitForm() {
      this.axios
        .put(
          cons.apis + "/permission/admins/" + this.edit_id + "/",
          {
