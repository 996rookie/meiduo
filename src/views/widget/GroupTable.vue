      ...
      <el-table-column label="操作" min-width="30">
        <template slot-scope="scope">
          <el-button size="mini" @click="fnPopShow(scope.row.id)"
            >修改</el-button
          >
          <el-button size="mini" type="danger" @click="fnDelGroup(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
      ...
      this.axios
        ...
        .then((dat) => {
          ...
          for (var i = 0; i < aList.length; i++) {
            aList2.push({ key: aList[i].id, label: aList2[i].name });
          }
    ...
    fnPopShow(id) {
      this.pop_show = true;
      this.axios
        .get(cons.apis + "/permission/groups/" + id + "/", {
          headers: {
            Authorization: "JWT" + token,
          },
          reponseType: "json",
        ...
        .then((dat) => {
          ...
          this.groupForm.listval = dat.data.permission;
        })
        .catch((error) => {
          console.log(error.response);
        });
    ...
    submitForm() {
      this.axios
        .put(
          cons.apis + "/permission/groups/" + this.edit_id + "/",
          {
    ...
    fnDelGroup(id) {
      ...
      this.$confirm("此操作将删除该组,是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .delete(cons.apis + "/permission/groups/" + this.edit_id + "/", {
              headers: {
                Authorization: "JWT" + token,
              },
              responseType: "json",
