<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="spu" label="SPU商品名称"></el-table-column>
      <el-table-column prop="spu_id" label="SPU商品id"></el-table-column>
    ...
    <el-dialog title="修改规格" :visible.sync="pop_show" append-to-body>
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button @click="resetForm('secpsForm')">重置</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    fnPopShow(id) {
      ...
      this.edit_id = id;
      this.axios
        .get(cons.apis + "/goods/sepcs/" + this.edit_id + "/", {
          headers: {
    ...
    submitForm() {
      this.axios
        .put(
          cons.apis + "/goods/specs/" + this.edit_id + "/",
          {
    ...
    fnDelSpecs(id) {
      ...
      this.$confirm("此操作将删除该规格,是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .delete(cons.apis + "/goods/specs/" + this.edit_id + "/", {
              headers: {
                Authorization: "JWT" + token,
              },
              responseType: "json",
