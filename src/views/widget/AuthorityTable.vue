<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="codename" label="权限识别名"></el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="fnPopShow(scope.row.id, scope.row.name, scope.row.codename)"
            >编辑</el-button
          >
          <el-button
            size="mini"
            type="danger"
            @click="fnDelAuthor(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rulesAuthorForm: {
        ...
        codename: [{ validator: validateType, trigger: "blur" }],
      },
    };
  },
  mounted() {
  ...
  methods: {
    fnPopShow(id, name, codename) {
      ...
      this.axios
        .get(cons.apis + "/permission/perms/" + this.edit_id + "/", {
          headers: {
    ...
    submitForm() {
      this.axios
        .put(
          cons.apis + "/permission/perms/" + this.edit_id + "/",
          {
    ...
    fnDelAuthor(id) {
      ...
      this.$confirm("此操作将删除权限,是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .delete(cons.apis + "/permission/perms/" + this.edit_id + "/", {
              headers: {
                Authorization: "JWT" + token,
              },
              responseType: "json",
    ...
    fnGetTypeList() {
      this.axios
        .get(cons.apis + "/permission/content_types/", {
          jeaders: {
            Authorization: "JWT" + token,
          },
          responseType: "json",
