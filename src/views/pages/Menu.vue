<template>
  <ul class="menu">
    <li v-for="(item, i) in aList" :key="i" @click="fnSetMenu([i, -1])">
      <router-link
        :to="item.url"
        class="level1"
        :class="i == aNowMenu[0] ? 'current' : ''"
      >
        <i class="fa" :class="aStylist[i]"></i>
        {{ item.name }}
      </router-link>
      <ul
        v-if="item.submenu.length > 0"
        :class="i == aNowMenu[0] ? 'current' + item.submenu.length : ''"
      >
        <li
          v-for="(subitem, j) in item.submenu"
          :key="j"
          @click.stop="fnSetMenu([i, j])"
        >
          <router-link
            :to="subitem.url"
            :class="j == aNowMenu[1] ? 'active' : ''"
          >
